...
const char* serverUrl = "https://unbiased-kite-apt.ngrok-free.app/upload/image";
const char* statusUrl = "https://unbiased-kite-apt.ngrok-free.app/upload/status";
const char* heartbeatUrl = "https://unbiased-kite-apt.ngrok-free.app/upload/heartbeat";
...
#define IDLE_TIMEOUT 7000000000

// New variables for interrupt and capture counting
volatile int captureCount = 0;
volatile unsigned long lastInterruptTime = 0;
const unsigned long debounceDelay = 1000; // 50ms debounce
...
float getInternalTemperature() {
    ...
    float tempC = raw * 5 / 9 - 40;
    return tempC;
}

// Status and Heartbeat (unchanged)
void sendStatusToServer() {
    if (!wifiConnected) return;
    float temperature = getInternalTemperature();
    String wifiStatus = (WiFi.status() == WL_CONNECTED) ? "Connected" : "Disconnected";
    String batteryPercent = "77";
    float heap = ESP.getFreeHeap();
    String cpuFrequency = String(ESP.getCpuFreqMHz());
...
void loop() {
    ...
    if (wifiConnected && captureCount > 0) {
    ...
    unsigned long elapsedTime = millis() - lastButtonPressMillis;
    if (!isIdle && elapsedTime >= IDLE_TIMEOUT) {
        Serial.println("Entering Idle Mode (Light Sleep)...");
        isIdle = true;
        delay(100);
        esp_light_sleep_start();
        // Check wake-up cause and increment captureCount if button triggered
