...
void uploadImageToServer() {
    ...
    camera_fb_t *fb = NULL;
    for (int i = 0; i < 3; i++) {  // Flush old frames
...
void loop() {
    ...
    // If button is pressed, do normal capture and reset idle timer
    if (wifiConnected && digitalRead(BUTTON_PIN) == LOW) {
    ...
    // Calculate how long since last button press
    unsigned long elapsedTime = millis() - lastButtonPressMillis;

    // 1. Enter Light Sleep (Idle) after 10s of no button press
    if (!isIdle && elapsedTime >= IDLE_TIMEOUT) {
        ...
        delay(100);
        // Enter Light Sleep - wakes instantly when button is pressed
        
        esp_light_sleep_start();
        coreID = xPortGetCoreID();
        sendStatusToServer();
        // After waking from light sleep, code resumes here
